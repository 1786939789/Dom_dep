...
function renameFuntion(node){
    var name = globalNameMaker.makeFunctionName();
    var parentNode = node.parent; //获取父节点
    var string = node.source().slice(0, 8) + " " + name + node.source().slice(8) + '\n'; //定义函数
    node.update(name); //更新函数辅助变量
    while(true){ //找到稳定父节点
        if(parentNode.type === 'Program' || parentNode.type === 'ExpressionStatement' || parentNode.type === 'VariableDeclaration'){
            break;
        }
        parentNode = parentNode.parent;
    }
    string += parentNode.source();
    parentNode.update(string);
}
...
module.exports = {
    phase0: function (code) {
        ...
        /* The reason of doing Phase 0B: It will be easier to track DOM dependancy */
